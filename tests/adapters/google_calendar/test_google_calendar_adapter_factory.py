...
# This file only verifies the Calendar-specific wire-up: that the factory
...
class TestHappyPath(_FactoryTestCase):
    def test_returns_wired_calendar_adapter(self) -> None:
        _save_creds(self.token_path, "hunter2")

        with patch(
            "mcp_server.adapters.google_calendar.google_calendar_adapter_factory.build"
        ) as mock_build:
        ...
        self.assertIsInstance(adapter, GoogleCalendarSourceAdapter)
        self.assertEqual(adapter.capabilities.source_name, "google_calendar")
